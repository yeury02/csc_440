...
def divide_half_by_x_coords(sorted_points):
	if len(sorted_points) > 6:
		...
		while sorted_points[m-1][0] == sorted_points[m][0]:
			m += 1
			#print(m)
		l = sorted_points[:m].copy()
		# high_x = points[m-1]
		# print(high_x)
		logging.info(l)

		r = sorted_points[m:].copy()
		logging.info(r)
...
def merge(left,right):
    ...
    while flag == False:
        flag = True
        while isClockwise(right[upperRight], left[upperLeft], left[(upperLeft + 1) % p1]) >= 0 :
            upperLeft = (upperLeft + 1) % p1

        while isClockwise(left[upperLeft], right[upperRight], right[(p2 + upperRight - 1) % p2]) <= 0 :
    ...
    while flag == False:
        flag = True
        while isClockwise(left[lowerLeft], right[lowerRight], right[(lowerRight + 1) % p2]) >= 0 :
            lowerRight = (lowerRight + 1) % p2

        while isClockwise(right[lowerRight], left[lowerLeft], left[(p1 + lowerLeft - 1) % p1]) <= 0 :
...
def computeHull(points):
	hull1 = base_case(points)
	return hull1

if __name__ == '__main__':

	#points = [(2,3),(3,4),(3,5),(5,6),(5,7),(5,8),(5,9),(6,8)]
	points = [(5,2),(2,2),(1,5),(0,2)]
	sorted_points = sort_by_x_coords(points)
	divide_half_by_x_coords(sorted_points)
	#hull1 = base_case(points)
	#computeHull(hull1)